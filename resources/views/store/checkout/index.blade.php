    ...
    <div class="page-wrapper">
        <div class="checkout shopping">
            <div class="container">
                <div class="row">
                    <div class="col-md-8">
                        <form action="{{route('checkout.address', $cart->id)}}" method="POST">
                            @csrf
                        ...
                        <div class="product-checkout-details">
                            <div class="block">
                                    ...
                                    <div class="media product-card">
                                        <a class="pull-left" href="product-single.html">
                                            <img class="media-object"
                                                 src="{{\App\Utils\File::getImage($product->product->images()->first())}}"
                                                 alt="Image"/>
                                        </a>
                                        <div class="media-body">
                                            <h4 class="media-heading"><a
                                                    href="product-single.html">{{$product->product->name}}</a>
                                            </h4>
                                            <p class="price">{{ $product->quantity }} x ${{$product->product->price}}</p>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <ul class="summary-prices">
                                    <li>
                                        <span>Subtotal:</span>
                                        <span class="price">${{$cart->cartPrices->subTotal}}</span>
                                    </li>
                                    <li>
                                        <span>Shipping:</span>
                                        <span>{{$cart->cartPrices->fees === 0 ? 'Free' : $cart->cartPrices->fees }}</span>
                                    </li>
                                    <li>
                                        <span>Discount:</span>
                                        <span>${{$cart->cartPrices->discount }}</span>
                                    </li>
                                </ul>
